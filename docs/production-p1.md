...
A second tool, `look_at_target` (P1 refinement), turns the head towards a point or an instance as one
COMBINE NLA layer on the head control's quaternion, measured on the deform bone. On the fixture a
walking character looks at a baton 1.2 m to its side: gaze error 37.8° before, 0.04° after (gate 2°),
largest head turn 4.8° per frame (gate 12°), revert restores 37.815° exactly. A target behind the
character (179.7°) is refused, a one-frame ramp (37.8° per frame) is not published, a second request
on the same target finds nothing to adjust. The frames were looked at: the head turns to the
character's right, the skin follows. Requests without `tool` remain `contact_lock` requests. Limits:
head only, not in the Director panel, not narrowable by custom tools.
